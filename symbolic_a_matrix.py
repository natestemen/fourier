...
def build_symbolic_a_matrix():
    ...
    # Given: h_i > 0, w_1 > w_2 > w_3 >= 1.
    # The formulas below additionally assume removing r_j does not merge blocks
    # (e.g., w1 - 1 != w2, w2 - 1 != w3) and does not delete a row (h_j > 1).
    ...
    removable_r1 = [
        (w1 - 1, h1 - 2),  # bottom of length-w1 block (requires h1 > 1)
        (w1 - 2, h1 - 1),  # bottom of new length-(w1-1) block
        r2,
        r3,
    ]
    ...
    removable_r2 = [
        r1,
        (w2 - 1, h1 + h2 - 2),  # bottom of length-w2 block (requires h2 > 1)
        (w2 - 2, h1 + h2 - 1),  # bottom of new length-(w2-1) block
        r3,
    ]
    ...
    removable_r3 = [
        r1,
        r2,
        (w3 - 1, h1 + h2 + h3 - 2),  # bottom of length-w3 block (requires h3 > 1)
