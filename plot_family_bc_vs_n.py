#!/usr/bin/env python3
"""Plot b(n) and |c(n)| for family w1=n,w2=2,w3=1,h1=h2=h3=1, including n→∞ limit."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/plots/family_bc_vs_n.png"),
    )
...
def _family_series(n_max: int, step: int, A_func, require_unitary: bool):
    ns: list[int] = []
    bs: list[float] = []
    cs: list[float] = []
    for n in range(50, n_max + 1, step):
...
def main() -> None:
    ...
    ns, bs, cs = _family_series(args.n_max, args.step, A_func, args.require_unitary)
    if not ns:
    ...
    ax.set_xlabel("n")
    ax.set_ylabel("value")
    ax.set_title("Family w1=n,w2=2,w3=1,h1=h2=h3=1")
