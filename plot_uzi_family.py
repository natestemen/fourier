#!/usr/bin/env python3
"""Plot b(n) and |c(n)| for Uzi family: w1=3,w2=2,w3=1,h1=1,h2=n,h3=1."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/plots/uzi_family_bc_vs_n.png"),
    )
...
def _family_series(n_min: int, n_max: int, step: int, A_func, require_unitary: bool):
    ...
    for n in range(n_min, n_max + 1, step):
        if n < 1:
            continue
        A_val = np.array(A_func(3, 2, 1, 1, n, 1), dtype=complex)
...
def main() -> None:
    ...
    ax.set_xlabel("n")
    ax.set_ylabel("value")
    ax.set_title("Uzi family: w1=3,w2=2,w3=1,h1=1,h2=n,h3=1")
